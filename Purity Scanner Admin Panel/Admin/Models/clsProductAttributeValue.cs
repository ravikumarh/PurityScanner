...
namespace Admin.Models
{
    public class clsProductAttributeValue
    {
        ...
        public int EditProductAttributeValue(clsProductAttributeValue tmpObj)
        {
            try
            {
                string str = "Select * from  ProductAttributeValues WHERE product_id=" + tmpObj.ProductID + " and sub_product_id=" + tmpObj.SubProductID + "and attribute_id=" + tmpObj.AttributeID + " and language_id=" + tmpObj.LanguageID + " and is_active=1";
                DataTable dtResult = DBobject.SelectData(str);
                if (dtResult.Rows.Count > 0)
                {
                    if (Convert.ToInt32(dtResult.Rows[0]["product_attribute_value_id"]) == tmpObj.ProductAttributeValuesID)
                    {
                        str = "update ProductAttributeValues set product_id=" + tmpObj.ProductID + ",sub_product_id=" + tmpObj.SubProductID + ",attribute_id=" + tmpObj.AttributeID + ",language_id=" + tmpObj.LanguageID + ",product_attribute_value='" + tmpObj.ProductAttributeValues + "',last_modified_date_time=Convert(datetime,'" + DateTime.Now + "') where product_attribute_value_id=" + tmpObj.ProductAttributeValuesID + "";
                        return DBobject.IUD_Data(str);
                    }
                    else
                    {
        ...
        public int AddProductAttributeValue(clsProductAttributeValue tmpObj)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count <= 0)
                {
                    ...
                    str = "insert into ProductAttributeValues(product_id,sub_product_id,attribute_id,language_id,product_attribute_value,last_modified_date_time,is_active) values(" + tmpObj.ProductID + "," + tmpObj.SubProductID + "," + tmpObj.AttributeID + "," + tmpObj.LanguageID + ",'" + tmpObj.ProductAttributeValues + "',Convert(datetime,'" + DateTime.Now + "'),1)";
                    return DBobject.IUD_Data(str);
                }
                else
                {
